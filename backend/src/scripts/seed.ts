...
export default async function seed({ container }: SeedParams) {
  ...
  // ---------------------------------------------------------------------------
  // 3c. Publishable API key — ensure a key exists linked to the default
  //     sales channel so /store/* requests don't 400.
  //
  // The storefront sends a hardcoded publishable key (pk_91ca8864... in
  // storefront/.env.local). v2 auto-generates the token on creation — we
  // cannot set a fixed value. We create the key, log the generated token,
  // and link it to the default sales channel. If the generated token does
  // NOT match the storefront's hardcoded value, update
  // storefront/.env.local NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY to the
  // logged token.
  // ---------------------------------------------------------------------------
  const apiKeyModule = resolve(Modules.API_KEY) as any

  // Look up by title + type. We CANNOT look up by the storefront's expected
  // token because v2 auto-generates the token on creation (CreateApiKeyDTO has
  // no `token` field), so a created key's token never equals the hardcoded
  // value. Matching on title keeps this idempotent across re-seeds.
  ...
  if (existingKeys.length > 0) {
    console.log(
      `Publishable API key already exists (token: ${existingKeys[0].token}) — skipping creation.`
    )
  } else {
    // Create a new publishable key. Token is auto-generated by the module.
    const [createdKey] = (await apiKeyModule.createApiKeys([
      {
        title: "Storefront publishable key",
        type: "publishable",
    ...
    console.log(
      `✓ Publishable API key created (token: ${createdKey.token})`
    )
    console.log(
      `  IMPORTANT: If this token does not match the value in storefront/.env.local,`
    )
    console.log(
      `  update NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY in storefront/.env.local to: ${createdKey.token}`
    )

    // Link the publishable key to the default sales channel.
    if (defaultSalesChannelId) {
      await remoteLink.create({
        [Modules.API_KEY]: { api_key_id: createdKey.id },
        [Modules.SALES_CHANNEL]: { sales_channel_id: defaultSalesChannelId },
      })
      console.log(
        `✓ Linked publishable API key to the default sales channel`
      )
    }
  }
